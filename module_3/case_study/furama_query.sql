...
where dv.ma_dich_vu not in (select distinct dv.ma_dich_vu
from dich_vu as dv
left join hop_dong as hd on hd.ma_dich_vu = dv.ma_dich_vu
...
-- nghĩa là tương ứng với mỗi tháng trong năm 2021 thì sẽ có bao nhiêu khách hàng thực hiện đặt phòng.
 ...
 select kh.ma_khach_hang, kh.ho_ten, dvdk.ten_dich_vu_di_kem
 from khach_hang as kh
 inner join loai_khach as lk on lk.ma_loai_khach = kh.ma_loai_khach
 inner join hop_dong as hd on hd.ma_khach_hang=kh.ma_khach_hang
 left join hop_dong_chi_tiet as hdct on hdct.ma_hop_dong = hd.ma_hop_dong
 ...
 create view w_dich_vu_chi_tiet_da_dat as
  select dvdk.ma_dich_vu_di_kem,sum(hdct.so_luong) as tong_so_luong
  from hop_dong_chi_tiet as hdct
  right join dich_vu_di_kem as dvdk on dvdk.ma_dich_vu_di_kem= hdct.ma_dich_vu_di_kem 
  group by dvdk.ma_dich_vu_di_kem;
  
select * from w_dich_vu_chi_tiet_da_dat;

select dvdk.*,wdv.tong_so_luong as so_luong_dich_vu_di_kem
from dich_vu_di_kem as dvdk
 left join w_dich_vu_chi_tiet_da_dat as wdv on wdv.ma_dich_vu_di_kem = dvdk.ma_dich_vu_di_kem
 where wdv.tong_so_luong = (select max(tong_so_luong) from w_dich_vu_chi_tiet_da_dat as wdv)
 group by dvdk.ma_dich_vu_di_kem;
 
 select dvdk.*,sum(hdct.so_luong) as tong_so_luong
 from hop_dong as hd
 left join hop_dong_chi_tiet as hdct on hdct.ma_hop_dong = hd.ma_hop_dong
 left join dich_vu_di_kem as dvdk on dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
 group by hdct.ma_dich_vu-di_kem
...
where year(hd.ngay_lam_hop_dong) in (2020,2021)
 ...
 delete from nhan_vien nv where not exists (select *
from hop_dong as hd 
where nv.ma_nhan_vien = hd.ma_nhan_vien and  year(hd.ngay_lam_hop_dong) in (2019,2020,2021));

-- 17.	Cập nhật thông tin những khách hàng có ten_loai_khach từ Platinum lên Diamond,
-- chỉ cập nhật những khách hàng đã từng đặt phòng với Tổng Tiền thanh toán trong năm 2021 là lớn hơn 10.000.000 VNĐ.
update loai_khach as lk set ten_loai_khach = 'Diamond'
 where exists (select *
 from hop_dong as hd
 left join hop_dong_chi_tiet as hdct on hdct.ma_hop_dong = hd.ma_hop_dong
...
select kh.*, sum(dv.chi_phi_thue+dvdk.gia * hdct.so_luong) as tong_tien
 from hop_dong as hd
 left join hop_dong_chi_tiet as hdct on hdct.ma_hop_dong = hd.ma_hop_dong
